...
num_write_access = [0] * (exe_time/EPOCH + 1)

cycle = 0
i = 0
print >>fout, "# Write Access density for blackscholes (for plotting)"
print >>fout, "# of write accesses within %s CPU cycle for %s (for plotting)" % (EPOCH, benchmark)
for line in ftrace:
    cycle = int(line.split()[0])
    while cycle/EPOCH != i:
        i += 1
    if int(line.split()[1]) == 1:
        num_write_access[i] += 1

for x in num_write_access:
    print >>fout, x
